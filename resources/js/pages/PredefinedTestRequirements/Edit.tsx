...
export default function EditPredefinedTest({ test, requirements }: Props) {
  ...
  // Initialiser le requirement sélectionné
  useEffect(() => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    put(route('predefined-tests-requirement.update', test.id), {
      preserveScroll: true,
      onSuccess: () => {
        // Optionnel : redirection après 1.4s pour voir le message de succès
        setTimeout(() => {
          router.visit(route('predefined-tests-requirement.index'))
        }, 1400)
      },
    })
  }
  ...
  return (
    <AppLayout
      breadcrumbs={[
        { title: 'Predefined Tests', href: route('predefined-tests-requirement.index') },
        { title: 'Edit', href: '' },
      ]}
    >
      ...
      <div className="min-h-full p-6 lg:p-10">
        ...
        <div className="flex items-center justify-between pb-6 border-b border-border/60 mb-10">
          ...
          <Button variant="outline" size="sm" asChild>
            <a href={route('predefined-tests-requirement.index')}>
        ...
        <form onSubmit={handleSubmit} className="space-y-10 max-w-5xl mx-auto">
          {/* Requirement (peut être non modifiable selon ta logique) */}
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Select
                ...
                disabled={true} // ← Option : rendre non modifiable si déjà lié
              >
          ...
          <div className="flex justify-end gap-4 pt-8 border-t border-border/60">
            <Button
              type="button"
              variant="outline"
              onClick={() => router.visit(route('predefined-tests-requirement.index'))}
              disabled={processing}
            >
              Cancel
            </Button>
