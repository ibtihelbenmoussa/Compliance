...
use App\Exports\PredefinedTestsExport; // à créer
...
class PredefinedTestRequirmentController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Tri (compatible avec react-table / tanstack)
        ...
        // Transformation légère pour Inertia (optionnel mais utile)
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('predefined-tests-requirement.index')
    ...
    public function update(Request $request, PredefinedTestRequirment $predefinedTest)
    {
        ...
        $validated = $request->validate([
            ...
            'objective'   => ['required', 'string'],
            'procedure'   => ['required', 'string'],
            // Si tu veux permettre de changer le requirement (rarement utile) :
            // 'requirement_id' => ['sometimes', 'exists:requirements,id', ...],
        ]);
    ...
    /**
     * Export Excel des tests prédéfinis (filtrés)
     */
    public function export(Request $request)
    {
        ...
        $query = PredefinedTestRequirment::query()
            ->with('requirement:id,code,title')
        ...
        if ($request->filled('search')) {
            ...
            $query->where(function ($q) use ($search) {
                $q->where('test_code', 'like', "%{$search}%")
                  ->orWhere('test_name', 'like', "%{$search}%")
                  ->orWhereHas('requirement', fn($sub) => $sub
    ...
    /**
     * Utilisé par le frontend pour récupérer le test prédéfini d'un requirement
     */
    ...
    // Helper pour factoriser la vérification d'accès
